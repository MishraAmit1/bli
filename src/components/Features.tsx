  ...
  return <>
    <section id="features" className="relative py-10 md:py-[70px] w-full">
      ...
      <Carousel
        ...
        opts={{
          loop: true,
          align: "start" // mobile par center me rakho
        }}
      ...
      >
        <div className="relative max-w-[1100px] mx-auto px-4">
          <CarouselContent className="flex gap-6 px-6 ml-2">
            {features.map((feature, index) => (
              <CarouselItem
                key={index}
                className={cn(
                  "sm:basis-1/2 lg:basis-1/3",
                  isMobile && "basis-[calc(100%-2rem)]"
                )}
              >
          ...
          <div className="text-center md:mt-18 mt-14">
            <div className="flex justify-center">
              <Button
                ...
                className="inline-flex items-center px-6 py-3 bg-[#113C6A] hover:bg-[#185EAA] text-[#FFFDF7] rounded-lg shadow-md hover:shadow-lg transition-all group"
              >
                View All Services
              ...
              <div className={cn("grid gap-1 mt-4 relative z-[50]", isMobile ? "grid-cols-2 gap-y-2" : "grid-cols-5")}>
                {sprintPhases.map((phase, index) => (
                  <HoverCard key={index} openDelay={0} closeDelay={0}>
                    ...
                    {/* Tooltip effect on hover wala jo hai jab mein hover karta hu to late aata hai muje jaldi chaiye on hover and haa <Progress></Progress> wala jo hai uski spped bhi kam karni hai  */}
                    <HoverCardContent className="z-[9999] w-64 shadow-xl border border-[#185EAA]/10 relative bg-white">
